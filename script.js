document.addEventListener('DOMContentLoaded', () => {
    ...
    function hidePreloader(callback) {
        let callbackCalled = false; // Flag to prevent double calls
    ...
    if (!hasPreloaderShown) {
        ...
        if (contentWrapper) contentWrapper.style.opacity = 0; // Ensure hidden before preloader starts
        animateKeywords(() => {
            hidePreloader(() => {
                showContent();
        ...
        if (contentWrapper) { contentWrapper.style.transition = 'none'; } // Prevent fade delay
        showContent();
    }
